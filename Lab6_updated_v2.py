...
#e)
#pdfore_daily = pdfore_2013_2014.groupby(['Date']).mean(numeric_only=True)
#print(forecast_cols)
pdfore_daily = pdfore.resample("D").mean()
pdfore_daily['is_holiday'] = np.where(pdfore_daily['Holiday'] == 1, 'Holiday', 'Non-Holiday')

demand_daily = pdfore_daily['Demand']
forecast_cols = ['1st naive_forecast', '2nd naive_forecast', '3rd naive_forecast']

for i, forecast_col in enumerate(forecast_cols):
    pdfore_daily[f'MAPE{i+1}'] = ((pdfore_daily[forecast_col] - demand_daily).abs() / demand_daily) * 100
...
for i, forecast_col in enumerate(forecast_cols):
    dfh = pdfore_daily.loc[pdfore_daily.is_holiday == 'Holiday']
    dfnh = pdfore_daily.loc[pdfore_daily.is_holiday == 'Non-Holiday']

    dfh.boxplot(column=[f'MAPE{i+1}'], by='is_holiday', ax=axes[i], positions=[2])
    dfnh.boxplot(column=[f'MAPE{i+1}'], by='is_holiday', ax=axes[i], positions=[1])

    axes[i].set_xticks([1, 2])
    axes[i].set_xticklabels(['Non-Holiday', 'Holiday'])
